...
def generate_cluster_dataset(
    ...
    n_cluster_points = n_rows - n_overlap - n_outliers
    cluster_weights = np.random.dirichlet(alpha=np.ones(n_clusters))
    cluster_sizes = (cluster_weights * n_cluster_points).astype(int)
    cluster_sizes[0] += n_cluster_points - cluster_sizes.sum()
    X = []
    y = []
    ...
    if n_outliers > 0:
        ...
        for i in indices:
            ...
            k = np.random.choice([1,2,3], size=1, p=[0.6, 0.3, 0.1])[0]
...
n_rows_dict = {'S' : 100,
               'M' : 1000}
n_clusters_dict = {'S': [3, 5],
                   'M': [5, 20],
                   'L': [20, 100]}
n_cols_list = [4, 8]
separation_dict = {'BASSA' : 1.5,
                    'MEDIA' : 3,
                    'ALTA' : 5}
separation_dict = {'BASSA' : 1.5,
                    'ALTA' : 5}
std_list = [0.5, 2]
outlier_scale_dict = [2, 4, 10]
rng = np.random.default_rng(42)
for dim, n_rows in n_rows_dict.items():
    out_dir = Path(Path(__file__).resolve().parent, 'dataframes/raw', dim)
    out_dir.mkdir(parents=True, exist_ok=True)
    for n_clusters in n_clusters_dict[dim]:
            ...
            for descr,separation in separation_dict.items():
                for std in std_list:
                    array, labels, couple = generate_cluster_dataset(n_rows=n_rows,
                                                                    ...
                                                                    n_outliers=int(n_rows/n_clusters),
                                                                    separation=separation,
                                                                    cluster_std=std,
                                                                    random_state=rng.integers(100,2000))
                    df = pd.DataFrame(array)
                    df['clusters'] = labels
                    output_path = Path(out_dir, f'df_cluster_{n_clusters}_sep_{descr}_std_{std}_cols_{n_cols}.csv')
...
# RPOVARE PIU VALORI DI RIGHE COLONNE STD SEP N-CLUSTER